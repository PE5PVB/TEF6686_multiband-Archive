      ...
      case RDS_GROUP_2B:
        {
          ...
          {
            int emptyLetters_bottom = 0;
            int emptyLetters_top    = 0;
            int textLen             = 0;

            textLen = strlen(rt_buffer);
            if (textLen > 0)
            {
              int i = 0;

              for (emptyLetters_bottom = 0;  rt_buffer[emptyLetters_bottom] <= ' '; emptyLetters_bottom++);
              for (emptyLetters_top    = 63; rt_buffer[emptyLetters_top] <= ' '; emptyLetters_top--);
              for (i = 0; i < 64; i++)
              {
                rds.stationText[i]  = 0;
                rt_buffer2[i]     = 0;
              }
              for (i = emptyLetters_bottom; i <= emptyLetters_top; i++)
              {
                rds.stationText[i - emptyLetters_bottom]   = rt_buffer[i];
                rds.stationText[i + 1 - emptyLetters_bottom] = 0;
              }
              ...
              for (i = 0; i < strlen(rds.stationText) && rds.stationText[i] == ' '; i++);
              if (i == strlen(rds.stationText))strcpy(rds.stationText, rds.stationName);
