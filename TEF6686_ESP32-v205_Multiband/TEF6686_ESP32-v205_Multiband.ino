...
void setup() {
  ...
  if (TEF == 205 && lowByte(device) == 14) {
    if (TEF != (highByte(hw) * 100 + highByte(sw))) {
      if ((highByte(hw) * 100 + highByte(sw)) != 205 && TEF != 102) {
        SetTunerPatch();  
      }
    }
  }
  ...
  if (menu == false && spec == false && manfreq == false)
  {
    ...
    if (strcmp(radio.rds.stationText, radioTextPrevious) || displayreset == true) {
      ...
      if (radio.rds.hasRT == true) {
        if (radio.rds.rtAB == 0) {
          Display.writeStr("AB.txt", "A");
        } else {
          Display.writeStr("AB.txt", "B");
        }
      } else {
        Display.writeStr("AB.txt", " ");
      }
...
void ShowSignalLevel()
{
  ...
  if (showrdsinfo == false) {
    if (USN < 250 && OStatus > -250 && OStatus < 250) {
...
void ModeButtonPress() {
  if (power == false) {
    if (menu == false) {
      ...
      if (counter - counterold <= 1000 && band != 5 ) {
        if (seek == true) {
          seek = false;
        }
        doTuneMode();
