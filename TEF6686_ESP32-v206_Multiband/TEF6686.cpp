...
void TEF6686::setFMSI_Gain(uint16_t band1, uint16_t band2, uint16_t band3, uint16_t band4) {
  devTEF_APPL_Set_StereoBandBlend_Gain(band1, band2, band3, band4);
}

void TEF6686::setFMSI_Bias(int16_t band1, int16_t band2, int16_t band3, int16_t band4) {
  devTEF_APPL_Set_StereoBandBlend_Bias(band1, band2, band3, band4);
}
      ...
      case RDS_GROUP_2B:
        {
          ...
          if (rt_process == 1)
          {
            ...
            strcpy(rds.stationText, rt_buffer);
          }
